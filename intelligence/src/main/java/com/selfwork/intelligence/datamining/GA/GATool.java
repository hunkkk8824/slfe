...
public class GATool {
    ...
    public GATool(String initStr) {
        if(!StringUtils.isEmpty(initStr)){
            String[] inits=initStr.split(" ");
            if(inits!=null && inits.length>=3){
                this.minNum=Integer.parseInt(inits[0]);
                this.maxNum =Integer.parseInt(inits[1]);
                this.initSetsNum = Integer.parseInt(inits[2]);
            }
        }
    ...
    public String geneticCal() {
        ...
        return printFinalCodes(initCodes);
    }
     ...
     */
    private String printFinalCodes(ArrayList<int[]> finalCodes) {
        int j = 0;
        StringBuilder builder = new StringBuilder();

        for (int[] array : finalCodes) {
            System.out.print("个体" + (j + 1) + ":");
            builder.append("个体" + (j + 1) + ":");
